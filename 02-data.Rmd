...
Now we have the data in an appropriate format for R purposes, we can proceed with some intial diagnostics

```{r}
df1 <-  merge(vix,ff,by= c("DATE" = "DATE"))

h = ggplot(df1,aes(CLOSE)) + geom_histogram(binwidth = 1,aes(y = ..density..)) +  geom_density(lwd = 1.5,color="blue") + stat_function(fun = dnorm, args = list(mean = mean(df1$CLOSE),sd = sd(df1$CLOSE)),color = "red", lwd = 1.5)
```
